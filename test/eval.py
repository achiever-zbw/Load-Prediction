...
def evaluate_main(model, test_loader, device, save_dir):
    ...
    针对 MainModel 接受 x_e , x_s , x_r , x_t
    """
    model.eval()
    all_preds = []
    all_targets = []
    ...
    with torch.no_grad():
        for be, bs, br, bt, target in test_loader:
            # LSTM Baseline 只需要 3 个输入
            output = model(be.to(device), bs.to(device), br.to(device) , bt.to(device))
            all_preds.append(output.cpu().numpy())
            all_targets.append(target.numpy())
...
def evaluate_nonn_channel_main(model, test_loader, device, save_dir):
    """
    1. 预测推理
    2. 反序列化 (Inverse Transform)
    3. 计算物理指标 (MAE, RMSE, MAPE)
    ...
    plt.title(f'MainModel - None Channel Attention ({len(targets_real)} points)', fontsize=14, fontweight='bold')
    plt.xlabel('Time Steps (5-min intervals)', fontsize=12)
    plt.ylabel('Cooling Load (kW)', fontsize=12)
    plt.legend(fontsize=11, loc='best')
    plt.grid(True, alpha=0.3)
