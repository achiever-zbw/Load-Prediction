...
from .eval import evaluate_lstm_baseline

def main():
    # 1. 配置
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    checkpoint_path = "saved/checkpoints/best_lstm_baseline.pth"
    scaler_dir = "saved/scaler/lstm_baseline"
    time_step = 24
    ...
    # 5. 模型初始化与权重加载
    model = LSTMBaseline(input_dim=10 , hidden_dim=64 , time_step=24)
    state_dict = torch.load(checkpoint_path, map_location=device)
    model.load_state_dict(state_dict)
    print(f"成功加载模型权重: {checkpoint_path}")

    # 6. 开始评估
    evaluate_lstm_baseline(model, test_dataloader, device, save_dir=scaler_dir , pic_name="LSTM Baseline")
