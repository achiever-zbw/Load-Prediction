...
def main():
    ...
    checkpoint_path = "saved/checkpoints/best_none_trans_main_model.pth"
    scaler_dir = "saved/scaler/none_trans_main"
    time_step = 24
    ...
    # 5. 模型初始化与权重加载
    model = NoneTransformerModel(dim=64 , time_step=24).to(device)
    state_dict = torch.load(checkpoint_path, map_location=device)
    model.load_state_dict(state_dict)
    print(f"成功加载模型权重: {checkpoint_path}")

    # 6. 开始评估
    evaluate_nonn_channel_main(model, test_dataloader, device, save_dir=scaler_dir , pic_name="None_Trans_Model")
