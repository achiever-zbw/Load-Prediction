...
def main():
    ...
    save_path = "saved/checkpoints/lstm_baseline_best.pth"
    ...
    # 保存 scaler
    os.makedirs("saved/scaler/lstm", exist_ok=True)
    joblib.dump(scaler_e, "saved/scaler/lstm/scaler_e.pkl")
    joblib.dump(scaler_s, "saved/scaler/lstm/scaler_s.pkl")
    joblib.dump(scaler_r, "saved/scaler/lstm/scaler_r.pkl")
    joblib.dump(scaler_y, "saved/scaler/lstm/scaler_y.pkl")
    ...
    # 4. 模型初始化
    model = LSTMBaseline(
        dim=64,           # 与 MainModel 一致
        hidden_dim=128,
        num_layers=2,
        dropout=0.2,
        time_step=24      # 与 MainModel 一致
    ).to(device)
    ...
    plt.title('LSTM Baseline - Training and Validation Loss', fontsize=14)
    plt.legend(fontsize=10)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()

    loss_curve_path = 'saved/lstm_baseline_loss_curve.png'
