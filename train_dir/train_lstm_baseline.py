...
def main() : 
    ...
    save_path = "saved/checkpoints/best_lstm_baseline.pth"
    ...
    scaler_dir = os.path.join("saved/scaler/lstm_baseline")
    ...
    train_dataset = NoneChannelAttnDataset(
        data_x=data_x[:train_size] , time_index=time_index[:train_size] , targets=data_y[:train_size] , time_step=24
    )
    ...
    # 4. model 初始化
    model = LSTMBaseline(input_dim=10 , hidden_dim=64 , time_step=24).to(device)
    loss_fn = nn.MSELoss()
